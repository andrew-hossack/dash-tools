...
def _add_changes_and_push_to_heroku(heroku_app_name: str) -> bool:
    ...
    print(f'dash-tools: heroku: deploy: Creating commit to push to Heroku')
    os.system(f'git add .')
    os.system(
        f'git commit -m "Deploy to Heroku for app {heroku_app_name} - dash-tools"')
    # Push to Heroku
    print(f'dash-tools: heroku: deploy: Pushing to Heroku')
    try:
...
def _remove_heroku_remote():
    ...
    Returns:
        ...
        False if failed
    """
    print('dash-tools: heroku: deploy: Removing existing heroku remote')
    try:
...
def _success_message(heroku_app_name: str):
    ...
    """
    print(f'\n\ndash-tools: heroku: deploy: Successfully deployed to Heroku from git branch "heroku"!')
    print(
        f'dash-tools: heroku: deploy: To push changes, select the "Update Existing App" option after typing: dash-tools --heroku: deploy')
    print(
        f'dash-tools: heroku: deploy: Management Page: https://dashboard.heroku.com/apps/{heroku_app_name}')
    print(
        f'dash-tools: heroku: deploy: Application Page: https://{heroku_app_name}.herokuapp.com/')

    # Prompt user to open the deployed app
    if input('dash-tools: heroku: deploy: Enter any key to open in browser or q to exit > ') != 'q':
...
def _get_valid_app_name() -> str:
    ...
    while not should_continue:
        ...
        if not herokuUtils.check_heroku_app_name_available(heroku_app_name):
            print(
                f'dash-tools: heroku: deploy: App "{heroku_app_name}" already exists on Heroku!')
            print(
                'dash-tools: heroku: deploy: Please choose a unique name that isn\'t already taken.')
            heroku_app_name = herokuUtils.get_heroku_app_name()
        elif not herokuUtils.validate_heroku_app_name(heroku_app_name):
            print(
                f'dash-tools: heroku: deploy: App name "{heroku_app_name}" is not valid!')
            print('dash-tools: heroku: deploy: Heroku app names must start with a letter, end with a letter or digit, can only contain lowercase letters, numbers, and dashes, and have a minimum length of 3 characters.')
            heroku_app_name = herokuUtils.get_heroku_app_name()
        else:
...
def deploy_app_to_heroku(project_root_dir: os.PathLike):
    ...
    if not herokuUtils.heroku_is_installed():
        print(f'dash-tools: heroku: deploy: Heroku CLI not installed!')
        print(f'dash-tools: heroku: deploy: See https://devcenter.heroku.com/articles/heroku-cli#install-the-heroku-cli')
        # Prompt user to open the deployed app
        if input('dash-tools: heroku: deploy: Enter any key to open in browser or q to exit > ') != 'q':
            webbrowser.open(
    ...
    if not gitUtils.git_is_installed():
        print(f'dash-tools: heroku: deploy: Git not installed!')
        print(f'dash-tools: heroku: deploy: See https://git-scm.com/downloads')
        # Prompt user to open the deployed app
        if input('dash-tools: heroku: deploy: Enter any key to open in browser or q to exit > ') != 'q':
    ...
    if not gitUtils.is_git_repository():
        print(f'dash-tools: heroku: deploy: Current directory is not a git repository!')
        print('dash-tools: heroku: deploy: To start a git repository, type: git init')
        exit('dash-tools: heroku: deploy: Failed')

    # Check that heroku remote is not already set
    if _check_heroku_remote_already_exists():
        print(f'dash-tools: heroku: deploy: Git remote "heroku" is already set!')
        print('dash-tools: heroku: deploy: Please choose an option below:')
        ...
        while not should_continue:
            ...
            if response == '1':
                ...
                print('dash-tools: heroku: deploy: Changes pushed to heroku remote')
                exit('dash-tools: heroku: deploy: Success')
            elif response == '2':
    ...
    if not _check_required_files_exist(project_root_dir):
        print('dash-tools: heroku: deploy: Procfile, runtime.txt, and requirements.txt are needed for Heroku deployment.')
        exit('dash-tools: heroku: deploy: Aborted')
    ...
    if not procfile['valid']:
        print(
            f'dash-tools: heroku: deploy: Procfile is incorrect. Did you include "{procfile["hook"]} = app.server" after instantiating "app = Dash(...)" in {procfile["dir"]}/{procfile["module"]}?')
        exit('dash-tools: heroku: deploy: Failed')

    # Log into Heroku
    if not herokuUtils.login_heroku_successful():
        print(f'dash-tools: heroku: deploy: Heroku login failed.')
        exit('dash-tools: heroku: deploy: Failed')
    ...
    print(f'dash-tools: heroku: deploy: Adding python buildpack')
